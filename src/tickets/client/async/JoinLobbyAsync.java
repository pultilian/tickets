...
public class JoinLobbyAsync /*extends AsyncTask<LobbyData, B, JoinLobbyResponse>*/ {
    ServerProxy proxy;
    ILobbyListPresenter callback;


    public JoinLobbyAsync(ServerProxy setProxy, ILobbyListPresenter setCallback) {
        proxy = setProxy;
    ...
    public JoinLobbyResponse doInBackground(LobbyData... data) {
        if (data.length != 1) {
            throw new Exception("Invalid user data passed to JoinLobbyAsync.doInBackground()");
        }
